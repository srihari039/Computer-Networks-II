	...
	def listen_from_peer(self,client,address):
		...
		while True:
			try:
				...
				# print('original_chain length : ',len(original_chain.chain))
				# print('last_hash - ',original_chain.chain[-1].hash)
			except:
				pass
				# print('--')
				# client.close()
...
# time.sleep(3)
# print(original_chain.chain[0].hash)

# time.sleep(5)
# # print('Broadcasting ...')
# peer_c_.broadcast_message('Hey cn!')
	...
	if mode == 'console':
		while True:
			...
			if opt == '1':
				...
				peer_c_.broadcast_message(UPDATED_CHAIN)
				cast_vote()
			else:
...
# print(original_chain.get_voting_details(user_.user_public_key))
# original_chain.cast_vote(new_vote)
...
# time.sleep(2)
# print(f'len of peers : {len(peers)}')
# peer_c_.broadcast_message(UPDATED_CHAIN)
